...
@Component
public class CtyptKisaSeedEcbTemplate {
	
	// KEY
	private static byte pbUserKey[] = {(byte)0xaa, (byte)0x59, (byte)0x3c, (byte)0xda, (byte)0xaf, (byte)0x99, (byte)0xd0, (byte)0xF1,
            (byte)0xE9, (byte)0x08, (byte)0x9e, (byte)0x42, (byte)0x0A, (byte)0xD4, (byte)0x05, (byte)0x89};
	
	public CtyptKisaSeedEcbTemplate() {}

		
	
	// -------------------------------------------------------- 데이터 양식 확인 ----------------------------------------
	
//	Map<String, String> cryptPatternMap = new HashMap<String, String>();
//	public boolean pattern_matches_value(String pattern, String value) {
		// 추가적인 정규표현식이 있으면 넣을것
//		cryptPatternMap.put("user_id", "^[0-9a-zA-Z]{6,10}$");
//		cryptPatternMap.put("passwd", "^[0-9a-zA-Z~!@#$%^&*]{9,20}$");
//		return Pattern.matches(cryptPatternMap.get(pattern), value);
//	}
	...
	*/
	...
	public String SHA256(String val) {
	 ...
	 * @return
	 */
	public String Encrypt_SEED(String value) {
		
		String encVal = "";
		try {
			byte[] encByte = value.getBytes();
			byte[] pbCipher = SEED_ECB_Encrypt(pbUserKey, encByte, 0, encByte.length);
			StringBuffer sb = new StringBuffer(pbCipher.length * 2);
		    String hexNumber;
		    for(int x = 0; x < pbCipher.length; x++) {
		    	hexNumber = "0" + Integer.toHexString(0xff & pbCipher[x]);
	 ...
	 * @return
	 */
	public String Decrypt_SEED(String value) {
		String decVal = "";
		...
		try {
		    ...
		    byte[] pbPlain = SEED_ECB_Decrypt(pbUserKey, ba, 0, ba.length);
		    decVal = new String(pbPlain);
		} catch(Exception e) {
			return value;
		}
